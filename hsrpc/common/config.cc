...
if (!name##_node) { \
  printf("Start hsrpc server error, failed to read node [%s]\n", #name); \
  exit(0); \
} \
...
#define READ_STR_FROM_XML_NODE(name, parent) \
  ...
  if (!name##_node|| !name##_node->GetText()) { \
    printf("Start hsrpc server error, failed to read config file %s\n", #name); \
    exit(0); \
  } \
...
Config::Config(const char* xmlfile) {
  ...
  if (!rt) {
    printf("Start hsrpc server error, failed to read config file %s, error info[%s] \n", xmlfile, xml_document->ErrorDesc());
    exit(0);
  }
